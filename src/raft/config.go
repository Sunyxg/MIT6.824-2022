...
type config struct {
	...
	connected   []bool   // 当前服务器是否在网络中
	saved       []*Persister
	endnames    [][]string            // the port file names each sends to
	logs        []map[int]interface{} // 每个服务器已经提交的日志的拷贝
	lastApplied []int
...
// 启动或重启一个Raft服务器，如果某个已经存在，则先杀死再重启
...
// 检查是否有一个服务器认为自己是leader
// 同时检查每个任期内是否只有一个leader
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		if index != -1 {
			...
			// 找到了raft集群中的leader并调用start提交了命令
			t1 := time.Now()
			for time.Since(t1).Seconds() < 2 {
				nd, cmd1 := cfg.nCommitted(index)
				if nd > 0 && nd >= expectedServers {
					// committed
					// 检查索引为index的Committed指令与start提交的指令是否相同
					if cmd1 == cmd {
